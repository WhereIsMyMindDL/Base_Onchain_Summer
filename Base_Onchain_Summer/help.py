...
logger.remove()
logger.add(stderr, format="<lm>{time:YYYY-MM-DD HH:mm:ss}</lm> | <level>{level}</level> | <blue>{name}:{function}:{line}</blue> | <lw>{message}</lw>")
logger.add('debug/logs - {time:DD-MM-YY}.log', format='<lm>{time:DD/MM/YY - HH:mm:ss!UTC}</lm> | <level>{level}</level> | <blue>{name}:{function}:{line}</blue> - <lw>{message}</lw>\n', rotation='1 day')
value_eth = [0.000032, 0.000063]                                      # минимальное и максимальное кол-во ETH для свапов и ликвы
...
waiting_gas = 3                                                     # макс значение газа при котором будет работать скрипт
...
RPC = {
    ...
    'Polygon': '',
    'Fantom': '',
    'Arbitrum one': 'https://rpc.ankr.com/arbitrum',
    'Avalanche': '',
...
def wait_gas():
    try:
        ...
        while True:
            ...
            if gasPrice < gas:
                logger.success(f"GWEI is normal | current: {gasPrice/10**9} < {gas/10**9}")
                break
...
def intro(count_wallets: int):
    print()
    print(f'Subscribe: https://t.me/CryptoMindYep')
    print(f'Total wallets: {count_wallets}\n')

    print()
    print(f'| {Fore.LIGHTGREEN_EX}Base_Onchain_Summer{Fore.RESET} |'.center(100, '='))
    print()
...
def get_tx_data(self, to, value=0, data=None):
    ...
    if False:
        ...
        max_fee = max_priority_fee + base_fee_per_gas - 1
        tx_data['maxPriorityFeePerGas'] = max_priority_fee
        tx_data['maxFeePerGas'] = max_fee
    else:
        tx_data['gasPrice'] = int(self.w3.eth.gas_price * 1.05)
